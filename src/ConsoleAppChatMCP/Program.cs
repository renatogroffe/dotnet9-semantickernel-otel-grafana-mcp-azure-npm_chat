...
await using var mcpClient = await McpClientFactory.CreateAsync(new StdioClientTransport(new()
{
    ...
    Command = "npx",
    Arguments = [
        $"@azure/mcp@{configuration["MCP:NpmPackageVersion"]}",
        "server", "start"
    ],
    EnvironmentVariables = new Dictionary<string, string?>
    {
        { "AZURE_TENANT_ID", configuration["MCP:AzureTenantId"] },
        { "AZURE_CLIENT_ID", configuration["MCP:AzureClientId"] },
        { "AZURE_CLIENT_SECRET", configuration["MCP:AzureClientSecret"] }
    }
}));
